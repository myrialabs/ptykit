/**
 * Runnable styled example (Bun).
 *
 *   bun examples/styled/server.ts
 *   # open http://localhost:8784
 *
 * Serves a single page that themes a terminal via `mountTerminal`'s style
 * options, and mounts the PtyKit WebSocket server at /pty. The browser bundle is
...
console.log(`styled running at http://localhost:${server.port}`);
